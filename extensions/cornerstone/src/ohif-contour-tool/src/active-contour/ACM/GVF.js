...
export const GVF = (function() {
  function GVF(params) {
    ...
    this.minlen = params.minlen || Math.pow(0.1, 2); //not used
    ...
    this.gradient = countChannelGradient(
      params.image,
      this.w,
      this.h,
      threshold
    ...
    for (let i = 0; i < this.snake.length; i++) {
      ...
      for (let dy = (wSize / 2) * -1; dy <= wSize / 2; dy++) {
        for (let dx = (wSize / 2) * -1; dx <= wSize / 2; dx++) {
          ...
          e_uniformity[1 + dx][1 + dy] = f_uniformity(
            prev,
            this.length,
            p,
            this.snake.length
          );
          e_curvature[wSize / 2 + dx][wSize / 2 + dy] = f_curvature(
            prev,
            p,
            next
          );
          e_flow[wSize / 2 + dx][wSize / 2 + dy] = f_ext(
            cur,
            p,
            dx,
            dy,
  ...
  function f_ext(cur, p, dx, dy, gflow_u, gflow_v, w, h) {
    if (p[0] >= w || p[0] <= 0) return 999;

    if (p[1] >= h || p[1] <= 0) return 999;
  ...
  function countGVF(image, w, h, threshold, mu) {
    //thresh == iter?
